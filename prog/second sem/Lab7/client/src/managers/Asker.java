...
public class Asker {
    ...
    public static Route askRoute(Console console, long id, long creatorId) throws AskBreak {
        ...
        try {
            ...
            if(!scrM) {
                return new Route(id, name, coordinates, from, to, distance, creatorId);
            }
            else{
                return new Route(id, name, coordinates, from, to, distance, creatorId);
            }
    ...
    public static Place askPlace(Console console) throws AskBreak, IOException {
        boolean scrM;
        scrM = MyConsole.getFileReader() != null;
        if(!scrM) {
    ...
    public static User askUser(Console console) throws IOException, AskBreak {
        ...
        if (!scrM) {
            while (true) {
                console.println("введите имя пользователя. Размер: не более 30. имя не может быть пустым.");
                name = console.readln().trim();
                if(name.equals("exit")) throw new AskBreak();
                if (!(name.isEmpty()) && (name.length() <= 30)) {
                    ...
                    console.println("имя не удовлетворяет условиям выше!!!");
                }
            }
        } else {
            name = console.filereadln().trim();
        ...
        String psw1;
        if(!scrM) {
            console.println("придумайте пароль!");
